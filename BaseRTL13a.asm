...
;DBGBP
          XOR EBX EBX
            ...
            Else_If  D@wParam = M00_Bin2BitsA
               call OnBin2Bits
            Else_If  D@wParam = M00_BitsA2Bin
               call OnBits2Bin
            Else_If  D@wParam = M00_AnyBits2BitsA
               call OnAnyBits2BitsA
            Else_If  D@wParam = M00_BitsA2AnyBits
               call OnBitsA2AnyBits
            Else_If  D@wParam = M00_AnyBits2OctalsA
               call OnAnyBits2OctalsA
            Else_If  D@wParam = M00_OctalsA2AnyBits
               call OnOctalsA2AnyBits
            ...
            Else_If  D@wParam = M00_AnyBitsFLTPrPtest
               mov ebx OnAnyBitsFermatLittleTheoremPrimarilityTest
...
Proc OnBin2Bits:
 cLocal @outSz @outMem @inSz @inMem
    ...
    call 'BaseCodecs.Binary2BitsA' D@outMem D@inMem D@inSz | test eax eax | je @BM
    mov D@outSz eax
    call ChooseAndSaveFileA D@outMem D@outSz
@BM:
    call VFree D@outMem | call VFree D@inMem
...
Proc OnBits2Bin:
 cLocal @outSz @outMem @inSz @inMem
    ...
    call ChooseAndLoadFileA | test eax eax | je P9>
    mov D@inMem eax, D@inSz edx
    ALIGN_ON 32 edx | shr edx 3
    mov D@outSz edx
    call VAlloc D@outSz | mov D@outMem eax | test eax eax | je @BM

    call 'BaseCodecs.BitsA2Binary' D@outMem D@inMem D@inSz | test eax eax | je @BM
    ALIGN_ON 8 eax | SHR eax 3 | mov D@outSz eax
    call ChooseAndSaveFileA D@outMem D@outSz
@BM:
    call VFree D@outMem | call VFree D@inMem
EndP
;
;
Proc OnAnyBits2BitsA:
 cLocal @outSz @outMem @inSz @inMem

    call ChooseAndLoadFileA | test eax eax | je P9>
    mov D@inMem eax, D@inSz edx
    add edx 4 | ALIGN_ON 4 edx | shl edx 3
    mov D@outSz edx
    call VAlloc D@outSz | mov D@outMem eax | test eax eax | je @BM
    mov eax D@inSz | SHL eax 3| ALIGN_ON 32 eax
    call 'BaseCodecs.AnyBits2BitsA' D@outMem D@inMem eax | test eax eax | je @BM
    mov D@outSz eax
    call ChooseAndSaveFileA D@outMem D@outSz
@BM:
    call VFree D@outMem | call VFree D@inMem
...
Proc OnBitsA2AnyBits:
 cLocal @outSz @outMem @inSz @inMem
    ...
    call 'BaseCodecs.BitsA2AnyBits' D@outMem D@inMem D@inSz | test eax eax | je @BM
    ALIGN_ON 32 eax | SHR eax 3 | mov D@outSz eax
    call ChooseAndSaveFileA D@outMem D@outSz
@BM:
    call VFree D@outMem | call VFree D@inMem
EndP
...
;
;
Proc OnAnyBits2OctalsA:
 cLocal @outSz @outMem @inSz @inMem
DBGBP
    call ChooseAndLoadFileA | test eax eax | je P9>
    mov D@inMem eax, D@inSz edx
    mov eax edx | mov ecx 3 | sub edx edx | SHL eax 3 | div ecx | add eax 8 | ALIGN_ON 4 eax
    mov D@outSz eax
    call VAlloc D@outSz | mov D@outMem eax | test eax eax | je @BM
    mov eax D@inSz | SHL eax 3
    call 'BaseCodecs.AnyBits2OctalsA' D@outMem D@inMem eax | test eax eax | je @BM
    mov D@outSz eax
    call ChooseAndSaveFileA D@outMem D@outSz
@BM:
    call VFree D@outMem | call VFree D@inMem
...
;
;
Proc OnOctalsA2AnyBits:
 cLocal @outSz @outMem @inSz @inMem

    call ChooseAndLoadFileA | test eax eax | je P9>
    mov D@inMem eax, D@inSz edx
    mov eax edx | mov edx 3 | mul edx | SHR eax 3 | add eax 4 | ALIGN_ON 4 eax
    mov D@outSz eax
    call VAlloc D@outSz | mov D@outMem eax | test eax eax | je @BM
DBGBP
    call 'BaseCodecs.OctalsA2AnyBits' D@outMem D@inMem D@inSz | test eax eax | je @BM
...
;
;
Proc OnAnyBitsFermatLittleTheoremPrimarilityTest:
 cLocal @inSz1 @inMem1 @Bit32 @numSz
  USES EDI
    call Input32BitNumber | cmp eax 0 | je P9>> | mov D@Bit32 eax
    call ChooseAndLoadFileA | test eax eax | je P9>>
...
call SetStartTick
    mov eax D@inSz1 | shl eax 3
    cmp D@Bit32 2 | je L0>
    call 'AnyBits.FermatLittleTheoremBase32bitPRPTest'  D@inMem1 EAX D@Bit32 | jmp L1>
L0: call 'AnyBits.FermatLittleTheoremBase2PRPTest'  D@inMem1 EAX
L1:
call SetEndTickReport
    cmp eax 0-1 | je L2> | cmp eax 1 | je L1>
L0:
    call 'User32.MessageBoxA' D$WindowHandle "Not a prime." "Fermat LT PRP test:" &MB_ICONINFORMATION | jmp @BM
L1:
    call 'User32.MessageBoxA' D$WindowHandle "Is Probably prime!" "Fermat LT PRP test:" &MB_ICONINFORMATION | jmp @BM
L2:
    call 'User32.SendMessageA', D$EDIT0_handle, &WM_SETTEXT, 0, "FLT PRP calculation Error!"
@BM:
    call VFree D@inMem1
EndP
;
...
Proc FermatUpto31BitsVirtualMod32Bit::
 ARGUMENTS @nBits @Bit32
 USES EBX
...
Proc FermatFrom32BitsVirtualMod32Bit::
 ARGUMENTS @Fm @Bit32
 cLocal @FmBitSz
 USES EBX ESI EDI
    ...
    mov ebx D@Fm | cmp ebx 32 | jb P9>>
    mov ecx ebx | shr ecx 5 ; create Fm exponent on stack
L0: push 0 | dec ecx | jns L0<
    mov ecx ebx | ALIGN_UP 32 ecx | mov esi esp | mov D@FmBitSz ecx
    BTS D$esi ebx
    call 'AnyBits.AnyBitsShiftRight' esi D@FmBitSz 5 | test eax eax | je P9>> ; Bits > Dword count
    mov ebx 1 ; HiBit always 1
    mov edi D$esi ; cut lowest dword to EDI
    call 'AnyBits.AnyBitsShiftRight' esi D@FmBitSz 32 | test eax eax | je P9>
    updateBitSize esi D@FmBitSz | cmp D@FmBitSz 32 | jne L1> | cmp D$esi 0 | je L3>
; 2^32 times division
L1: mov ecx D@Bit32
    mov edx ebx
L0: sub eax eax | DIV ecx | dec edi | jne L0<
    mov ebx edx

    call 'AnyBits.AnyBitsSubstractSelf32Bit' esi D@FmBitSz 1 | cmp D$esi 0 | jne L1<
    updateBitSize esi D@FmBitSz | cmp D@FmBitSz 32 | jne L1<
; last count-1 times division
L3: dec edi
    mov ecx D@Bit32
    mov edx ebx
L0: sub eax eax | DIV ecx | dec edi | jne L0<
    mov eax 1 ; last div with 1 Lo32
    DIV ecx
    mov eax 1; EDX=Mod32
EndP
...
L1: cmp esi 32 | jae L5>
L1:
cmp D$JobAskToEnd &TRUE | je L2>>
    call 'NUMERIKO.GetNextPrimeFromSieve' D$SieveBase0 EBX | mov ebx eax | cmp eax 0 | je L3>
    call FermatUpto31BitsVirtualMod32Bit edi ebx | cmp eax 1 | jne L2>
    test edx edx | jne L1<
    jmp L3>
L5:
cmp D$JobAskToEnd &TRUE | je L2>
    call 'NUMERIKO.GetNextPrimeFromSieve' D$SieveBase0 EBX | mov ebx eax | cmp eax 0 | je L3>
;    mov edi pReportBuffer | call Dword2Decimal edi ebx | add edi eax | mov eax 0A0D | stosd
;    call 'User32.SendMessageA', D$EDIT0_handle, &WM_SETTEXT, 0, pReportBuffer
    call FermatFrom32BitsVirtualMod32Bit esi ebx | cmp eax 1 | jne L2>
    test edx edx | jne L5<
;    jmp L3>
L3:
call SetEndTick
...
Proc dumpNModsAny:
 ARGUMENTS @pNMod @NModSz
  USES ESI EDI
    ...
    call 'BaseCodecs.AnyBits2HexA' esi D@pNMod D@NModSz | add edi eax
    mov eax 0A0D | stosw
    mov B$edi 0
    sub edi esi
    call 'BaseRTL.LogAppendToBuffer' LogFileStruct esi edi
